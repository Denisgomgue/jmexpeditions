...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['id_paquete']) && isset($_POST['id_destino']) && isset($_POST['orden_itinerario']) 
        && isset($_POST['duracion_visita']) && isset($_POST['tipo_destino']) 
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO itinerarios (id_paquete, id_destino, orden_itinerario, duracion_visita, tipo_destino, descripcion_actividad) 
                                   VALUES (?, ?, ?, ?, ?, ?)");
            if ($stmt->execute([$id_paquete, $id_destino, $orden_itinerario, $duracion_visita, $tipo_destino, $descripcion_actividad])) {
